...
{	end else
  ...
  try
    EnvFile := TIniFile.Create(ENVPATHFILE);
		...
		GS_PRJ_AREA := 'S';   // 원주 365 system 수도권 전용
...
//            1202: TCK_USER_PER.SET_Accept := '1';                   // 설정 : 접수설정
        ...
        try
          ...
          if GB_365System then ln_envfile.WriteString('CHARGE', 'CH_GUBUN', 'O'); // 원주365는 무조건 온라인 요금 서버로 접속
...
//          end;
          ...
          // GS_SEARCH_MORE_FAST 삭제 - N사 API키 서비스 중지로 인해 다음 API키를 사용하지 않으면 API키를 사용 안함으로 변경  20200608  LYB
					...
					GS_ADMINMAP_CHK := ln_envfile.ReadString('SearchWKSet',     'ADMINMAP_CHK', '1');    // 1.자기사 기본선택  정회귀팀장요청 20201118 LYB
...
const
  ...
  _PUBLIC_IP_ERROR = '홈페이지 호출 중 오류가 발생했습니다.'#13#10 +
