...
type
  Tfrm_AIC09 = class(TForm)
    ...
    cxGridAI2_List3Column3: TcxGridDBColumn;
    cxGrid1: TcxGrid;
    ...
    cxGridAI2_List2Column6: TcxGridDBColumn;
...
procedure Tfrm_AIC09.BtnCloseClick(Sender: TObject);
begin
	close;
end;

procedure Tfrm_AIC09.btnExitClick(Sender: TObject);
begin
...
procedure Tfrm_AIC09.cxGridAI2_List2CellClick(Sender: TcxCustomGridTableView;
	ACellViewInfo: TcxGridTableDataCellViewInfo; AButton: TMouseButton;
  AShift: TShiftState; var AHandled: Boolean);
var 	XmlData, Param, ErrMsg, ls_MSG_Err : string;
	...
	tmpCnt : Integer;
begin
	SetDebugeWrite('Tfrm_AIC09.cxGridAI2_List2CellClick');
	Try
        ...
        begin
				...
				try
					...
					// 순번 | 통화시작시간 | 통화종료시간 | 통화구분 | 차감캐쉬 | 녹취내역 | 저장일시
						...
						cxGridAI2_List3.DataController.Values[iRow, 6] := '';   //녹취청취
						cxGridAI2_List3.DataController.Values[iRow, 7] := '';   //다운로드
						cxGridAI2_List3.DataController.Values[iRow, 8] := ls_Rcrd[5];   //저장일시
					end;
				finally
					ls_Rcrd.Free;
				end;
...
procedure Tfrm_AIC09.cxGridAI2_List2Column2PropertiesButtonClick(
  Sender: TObject; AButtonIndex: Integer);
Var iRow, iCol, iDate, iTime : Integer;
...
procedure Tfrm_AIC09.cxGridAI2_List2Column5PropertiesButtonClick(
  Sender: TObject; AButtonIndex: Integer);
Var iRow : Integer;
	sUrl : string;
	iCol, iDate, iTime : Integer;
	sFileNm, sDate, sPPN, sYMD : String;
begin
	SetDebugeWrite('Tfrm_AIC09.cxGridAI2_List2Column5PropertiesButtonClick');
	Try
		iRow := cxGridAI2_List2.DataController.FocusedRecordIndex;
		if iRow  = -1 then Exit;
		iDate := cxGridAI2_List2.GetColumnByFieldName('통화시작시간').Index;
...
procedure Tfrm_AIC09.cxGridAI2_List3Column2PropertiesButtonClick(
  Sender: TObject; AButtonIndex: Integer);
Var iRow : Integer;
	sUrl : string;
	iCol, iDate : Integer;
	sFileNm, sDate, sPPN, sYMD : String;
begin
	SetDebugeWrite('Tfrm_AIC09.cxGridAI2_List3Column2PropertiesButtonClick');
	Try
    ...
    iCol := cxGridAI2_List3.GetColumnByFieldName('ppn').Index;
		...
		sUrl := fgetAiRecordURL(sDate, sPPN);
		sFileNm := sYMD + '-' + cxGridAI2_List3.DataController.Values[iRow, iCol] + '.wav';
...
procedure Tfrm_AIC09.cxGridDBColumn6PropertiesButtonClick(Sender: TObject;
	AButtonIndex: Integer);
...
procedure Tfrm_AIC09.FormActivate(Sender: TObject);
begin
  cxLblActive.Color := GS_ActiveColor;
	cxLblActive.Visible := True;
end;

procedure Tfrm_AIC09.FormClose(Sender: TObject; var Action: TCloseAction);
begin
	Action := caFree;
end;

procedure Tfrm_AIC09.FormCreate(Sender: TObject);
var i : integer;
...
procedure Tfrm_AIC09.FormDeactivate(Sender: TObject);
begin
	cxLblActive.Visible := False;
end;

procedure Tfrm_AIC09.FormDestroy(Sender: TObject);
begin
	Frm_AIC09 := Nil;

end;

procedure Tfrm_AIC09.FormShow(Sender: TObject);
begin
  fSetFont(Frm_AIC09, GS_FONTNAME);
end;

function Tfrm_AIC09.func_FileDown(ALink: string): Boolean;
...
function Tfrm_AIC09.func_FileStream(AFileNm, AUrl: String): Boolean;
...
Var
  sUrl, sFileName : string;
	...
	sFileNm : String;
begin
	SetDebugeWrite('Tfrm_AIC09.func_FileStream');
	Try
		Result := False;
		sUrl := AUrl;
		sFileNm := AFileNm;
...
procedure Tfrm_AIC09.pnlTitleMouseMove(Sender: TObject; Shift: TShiftState; X,
  Y: Integer);
begin
  ReleaseCapture;
...
procedure Tfrm_AIC09.pPlay(sDate, sPPN : String);
...
procedure Tfrm_AIC09.proc_Get_List(AList: string);
var	XmlData, ls_MSG_Err : string;
	...
	ls_Rcrd : TStringList;
begin
	SetDebugeWrite('Tfrm_AIC09.proc_Get_List');
	Try
		XmlData := AList;
		xdom := ComsDomDocument.Create;
		cxGridAI2_List2.DataController.SetRecordCount(0);
				...
				begin
					...
					try
						...
						// 순번 | 통화시작시간 | 통화종료시간 | 통화구분 | 차감캐쉬 | 녹취내역
							...
							cxGridAI2_List2.DataController.Values[iRow, 8] := '';     //녹취청취
							cxGridAI2_List2.DataController.Values[iRow, 9] := '';     //다운로드
							cxGridAI2_List2.DataController.Values[iRow, 10] := ls_Rcrd[7];        //저장일시
						end;
					finally
						ls_Rcrd.Free;
					end;
...
procedure Tfrm_AIC09.proc_Init;
var i : integer;
begin
	cxGridAI2_List2.DataController.SetRecordCount(0);
	for i := 0 to cxGridAI2_List2.ColumnCount - 1 do
