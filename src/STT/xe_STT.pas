...
type
  ...
  private
    ...
    iCaCnt, iToCnt : Integer;
		...
		procedure RequestDataOrderGradeList;
	public                                  
		{ Public declarations }
...
begin
  ...
  cxPageControl1.ActivePageIndex := 0;
	...
	cxPageControl1.Pages[15].TabVisible := (TCK_USER_PER.STT_OrderGrade  = '1');  // 714.오더별평가조회
...
begin
  try
    ...
    cbbA16.Properties.Items.Clear;
    cbbA16.Properties.Items := cbbA4.Properties.Items;
    cbbA16.ItemIndex := cbbA4.ItemIndex;
    cbbA16.Enabled   := cbbA4.Enabled;
	end;
end;
...
//				0:
  ...
  // --------------------------------------------------------------------------- A10
	...
	// --------------------------------------------------------------------------- A16
	try
		for I := 0 to cxGridDBTableView16.ColumnCount - 1 do
		begin
			case I of
				0, 6:
					begin
						cxGridDBTableView16.Columns[I].DataBinding.ValueTypeClass := TcxCurrencyValueType;
						cxGridDBTableView16.Columns[I].Properties := frm_Main.gCurProperties;
					end;
			else
				cxGridDBTableView16.Columns[I].DataBinding.ValueTypeClass := TcxStringValueType;
			end;
		end;
		cxDtStartA16.Date := StrToDate(Date8to10(copy(StartDateTime('yyyymmddhhmmss'), 1, 8))) - 7;
		cxDtEndA16.Date := cxDtStartA16.Date + 7;
		gSortIdx := 0;
	except
	end;
					...
					try
						...
						begin
							...
							begin
								...
								cxGridA15View1.DataController.SetValue(iNum, 12, StrToInt64Def(ls_Rcrd[13], 0));                                  // 운행요금
								cxGridA15View1.DataController.SetValue(iNum, 13, StrToInt64Def(ls_Rcrd[14], 0));                                  // 수수료
...
procedure TFrm_STT.RequestDataOrderGradeList;
var
	XmlData, Param, ErrMsg, ls_MSG_Err : string;
	ErrCode, i, j, iTmp, iRow : Integer;
	xdom: msDomDocument;
	lst_Result : IXMLDomNodeList;
	ls_Rcrd,slList : TStringList;
	strHdNo, strBrNo, sKeyNum, sHdNo, sBrNo, sHdNm, sBrName, sName : string;
begin
	SetDebugeWrite('TFrm_STT.RequestDataOrderGradeList');
	...
	try
		...
		Param := Param + '│' + strBrNo;
		Param := Param + '│' + formatdatetime('yyyymmdd', cxDtStartA16.Date) + '│' + formatdatetime('yyyymmdd', cxDtEndA16.Date);
		...
		slList := TStringList.Create;
		try
			if not RequestBasePaging(GetSel06('GET_WK_EVALUATION_LIST', 'CAS.GET_WK_EVALUATION_LIST', '1000', Param), slList, ErrCode, ErrMsg) then
			begin
        Frm_Flash.hide;
        Timer1.Enabled := False;
        Application.ProcessMessages;
        GMessagebox(Format('오더별 평가 리스트 오류'#13#10'[%d]%s', [ErrCode, ErrMsg]), CDMSI);
        Frm_Main.Enabled := True;
        Screen.Cursor := crDefault;
        Exit;
			end;
			...
			try
				...
				cxGridDBTableView16.DataController.SetRecordCount(0);
				cxGridDBTableView16.BeginUpdate;
				for j := 0 to slList.Count - 1 do
				begin
            ...
            begin
              ...
              try
                ...
                begin
                  ...
                  if not Check_ID(GT_USERIF.ID, ls_Rcrd[2]) then
                  begin
                    //70089667│A100│B100││01089701784│34223*정경석│3│1111│2021-10-28 17:00:47
                    iRow := cxGridDBTableView16.DataController.AppendRecord;
                    ls_Rcrd.Insert(0, IntToStr(iRow + 1));
                    cxGridDBTableView16.DataController.SetValue(iRow, 0, ls_Rcrd[0]);     //순번
                    sHdNo := ls_Rcrd[2];
                    sBrNo := ls_Rcrd[3];
                    iTmp := scb_BranchCode.IndexOf(sBrNo);
                    if iTmp >= 0 then
                      sBrName := scb_BranchName[iTmp]
                    else
                      sBrName := '';
                    sName := '[' + sHdNo + '] - [' + sBrNo + ']' + sBrName;
                    
                    cxGridDBTableView16.DataController.SetValue(iRow, 0, ls_Rcrd[0]);     //순번
                    cxGridDBTableView16.DataController.SetValue(iRow, 1, sName);          //지사코드
                    cxGridDBTableView16.DataController.SetValue(iRow, 2, ls_Rcrd[1]);     //접수번호
                    cxGridDBTableView16.DataController.SetValue(iRow, 3, ls_Rcrd[4]);     //어플명
                    cxGridDBTableView16.DataController.SetValue(iRow, 4, StrToCall(ls_Rcrd[5]));     //고객번호
                    cxGridDBTableView16.DataController.SetValue(iRow, 5, ls_Rcrd[6]);     //운행기사명
                    cxGridDBTableView16.DataController.SetValue(iRow, 6, StrToFloatDef(ls_Rcrd[7], 0));     //평가점수
                    cxGridDBTableView16.DataController.SetValue(iRow, 7, ls_Rcrd[8]);     //평가메모
                    cxGridDBTableView16.DataController.SetValue(iRow, 8, GetStrToDateTimeGStr(ls_Rcrd[9], 4));     //등록일
                  end;
                end;
              finally
                ls_Rcrd.Free;
...
procedure TFrm_STT.btnSearchA16Click(Sender: TObject);
begin
	if (GT_SEL_BRNO.GUBUN = '0') and (not Check_ALLHD(GT_SEL_BRNO.HDNO)) then
	begin
		GMessagebox('해당본사에 대한 전체 권한이 없습니다.' + #13#10 + '지사를 선택하여 주십시오.', CDMSI);
    ...
    10:
      begin
       ...
       cxDtEndA10.Date := AEnd;
		...
		15:
			begin
			 cxDtStartA16.Date := AStart;
			 cxDtEndA16.Date := AEnd;
			end;
...
procedure TFrm_STT.btnExcelA16Click(Sender: TObject);
begin
  if cxGridDBTableView16.DataController.RecordCount = 0 then
  begin
    GMessagebox('자료가 없습니다.', CDMSI);
    Exit;
  end;
  ...
  Frm_Main.sgExcel := '통계-오더별평가조회.xls';
  Frm_Main.sgRpExcel := Format('통계>오더별평가조회]%s건', [GetMoneyStr(cxGrid_OrderTimeList.DataController.RecordCount)]);
  Frm_Main.cxGridExcel := cxGridA16;
  Frm_Main.cxGridDBViewExcel := cxGridDBTableView16;
  Frm_Main.proc_excel(0);
end;
