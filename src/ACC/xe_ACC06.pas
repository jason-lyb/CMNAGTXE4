...
procedure TFrm_ACC06.FormActivate(Sender: TObject);
begin
  cxLblActive.Color := GS_ActiveColor;
  cxLblActive.Visible := True;
end;

procedure TFrm_ACC06.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  Action := caFree;
end;

procedure TFrm_ACC06.FormCreate(Sender: TObject);
var
...
procedure TFrm_ACC06.FormDeactivate(Sender: TObject);
begin
  cxLblActive.Visible := False;
end;

procedure TFrm_ACC06.FormDestroy(Sender: TObject);
begin
  Frm_ACC06 := Nil;
end;

procedure TFrm_ACC06.btnCloseClick(Sender: TObject);
begin
  Close;
end;

procedure TFrm_ACC06.FormShow(Sender: TObject);
begin
  fSetFont(Frm_ACC06, GS_FONTNAME);
  PnlTitle.Caption := '   콜대행수수료(기간검색)_상세'
end;

procedure TFrm_ACC06.proc_init;
begin
  try
    // 그리드 초기화
...
procedure TFrm_ACC06.Grid3ViewColumnHeaderClick(Sender: TcxGridTableView;
  AColumn: TcxGridColumn);
begin
  AIndex := AColumn.Index;
end;

procedure TFrm_ACC06.Grid3ViewDataControllerSortingChanged(Sender: TObject);
begin
  gfSetIndexNo(Grid3View, AIndex, GS_SortNoChange);
end;

procedure TFrm_ACC06.cxButton1Click(Sender: TObject);
begin
  ...
  Frm_Main.sgRpExcel := Format('정산>콜대행수수료(기간)_상세]%s건', [GetMoneyStr(Grid3View.DataController.RecordCount)]);
  Frm_Main.cxGridExcel := Grid3;
  Frm_Main.bgExcelOPT := False;
  Frm_Main.proc_excel(0);
end;

procedure TFrm_ACC06.pnlTitleMouseDown(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
  ReleaseCapture;
