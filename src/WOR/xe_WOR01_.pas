...
//  cxedSoNum.Enabled := True;
  ...
  cxtP1.Text := ''; cxtP2.Text := '';cxtP3.Text := '';cxtP4.Text := '';
	...
	if cxBrNo.Text <> '' then
	begin
		sTemp := GetPlusCallYN(cxBrNo.Text);
		if copy(sTemp,2,4) = 'yyyy' then
		begin
...
//- wk_ins_type 추가 'b' 
		{m:단체보험(월당), c:콜당(메리츠), d: db손보(개인보험), k:콜당(KB), n:미지정, b:보험개발원연동(개인보험)}
		  ...
		  if cb_Shuttle_Status.ItemIndex = 2 then
			...
			Try
				sTmp := '';
				if cb_Wor23List.ItemIndex < 0 then sTmp := '' else sTmp := slMCashSeq[cb_Wor23List.ItemIndex];
			except
				sTmp := '';
			End;
...
var iTmp : integer;
begin
	iTmp := Pos(',', cboBranch.Text);
	Result := Copy(cboBranch.Text, iTmp + 1 , 4); 
...
begin
	if scb_BranchCode.Count <= 0 then
		Exit;
...
begin
	...
	if GS_PRJ_AREA = 'O' then
	begin
		if ((GT_USERIF.ShareNo = 'G05') or (GT_USERIF.ShareNo = 'G12') or (GT_USERIF.ID = 'sntest')) then
...
var
	ls_TxLoad, sNode, sHp, rv_str, sMsg: string;
  ls_rxxml: WideString;
  xdom: msDomDocument;
  lst_Node: IXMLDOMNodeList;
  slReceive: TStringList;
...
begin
	...
	try
		...
		sNode := '/cdms/header/ClientKey';
		lst_Node := xdom.documentElement.selectNodes(sNode);
		lst_Node.item[0].attributes.getNamedItem('Value').Text := 'WOR015';
		sNode := '/cdms/Service/Data/Query';
		lst_Node := xdom.documentElement.selectNodes(sNode);
		lst_Node.item[0].attributes.getNamedItem('Key').Text := 'WKHPCHK2' ;   //지방권의 경우에난 기존 전문
    ...
    if IsHBShare(GS_PRJ_AREA = 'O', GT_USERIF.ShareNo) then sHp := sHp + '#3' else
    if GB_365System then sHp := sHp + '#7' else  //#7 유효성체크
    if GB_CallLine  then sHp := sHp + '#5' else  //#5 유효성체크
		if FShuttel4[cboBranch.itemindex] = 'y' then sHp := sHp + '#4'; //#4 셔틀기사 유효성체크

		iUse_Chk := TcxButton(sender).Tag;
		lst_Node.item[0].attributes.getNamedItem('Value').Text := sHp;
...
procedure TFrm_WOR01.ChkHPUseYnO;
var
  ls_TxLoad, sNode, sHp, rv_str: string;
	ls_rxxml: WideString;
  xdom: msDomDocument;
  lst_Node: IXMLDOMNodeList;
  slReceive: TStringList;
...
begin
	iTmp := Pos(',', cboBranch.Text);
	Result := Copy(cboBranch.Text, 2, iTmp-2); //'(A100,O501)SN테스트지사_01/콜마너본사'
end;
