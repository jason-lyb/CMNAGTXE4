...
uses
  ...
  dxSkinSharp, dxSkinMetropolisDark, dxSkinOffice2007Silver, cxProgressBar, cxMemo,
...
type
  TFrm_WORA1 = class(TForm)
    ...
    cxLabel171: TcxLabel;
    cxgWkCounselColumn72: TcxGridDBColumn;
    cxgWkCounselColumn73: TcxGridDBColumn;
    cxLabel23: TcxLabel;
    grpBar: TcxGroupBox;
    pBar: TcxProgressBar;
    grpStateChange: TcxGroupBox;
    meoWkList: TcxMemo;
    btnChange: TcxButton;
    cxButton9: TcxButton;
    lblState: TcxLabel;
    cxgWkCounselColumn74: TcxGridDBColumn;
    cxgWkCounselColumn77: TcxGridDBColumn;
    cxgWkCounselColumn78: TcxGridDBColumn;
    ...
    procedure proc_SelectSettlementExec(sKey : string; AWkList : TStringList; bEnd, bDivision : Boolean);
...
uses xe_Dm, xe_Func, xe_gnl2, xe_gnl3, xe_xml, xe_Msg, xe_Lib, Main,
 ...
 const
	 WorA1SL : array[0..81] of TSortList = (
     (cName : 'No'               ;  cWid : 050 ),
     (cName : '지사명'           ;  cWid : 100 ),
     (cName : '기사사번'         ;  cWid : 060 ),
     (cName : '자체사번'         ;  cWid : 060 ),
...
begin
	try
      ...
      cxgWkCounsel.Columns[iRow].DataBinding.ValueType := 'Currency';
			...
			//보이는 칼럼은 Tag = 1, 숨긴칼럼은 Tag = 0
			iRow := cxgWkCounsel.GetColumnByFieldName('현장카드').Index;
			...
			if (GS_PRJ_AREA = 'O') and (GT_USERIF.ShareNo <> 'G05') and (GT_USERIF.ShareNo <> 'G12') and (GT_USERIF.ShareNo <> 'G29') and (GT_USERIF.ID <> 'sntest') then
			begin                       //제주연합                       //울산연합
...
//      keyword_text01.Width := 120;
      ...
      sWkSearchBrNo := '';
			...
			begin
				...
				cbWKInsGubun.Properties.Items.Add('개인보험');
			end else
			begin
				cbWKInsGubun.Properties.Items.Add('전체');
				cbWKInsGubun.Properties.Items.Add('월정액보험');
...
procedure TFrm_WORA1.btnChangeClick(Sender: TObject);
Var sKey : String;
begin
  grpStateChange.Visible := False;
  ...
  begin
    sKey := IntToStr(grpStateChange.Tag);
...
var
  ...
  ls_Rcrd: TStringList;
	...
	iwkTitle: array[0..84] of integer;
	ls_rxxml: WideString;
...
//                          cxGLVWK.DataController.Values[iRowe42]] := StrToIntEx(ls_Rcrd[3]);
  ...
  //												if (GS_PRJ_AREA = 'O') and ((GT_SEL_BRNO.HDNO = 'A922') OR (GT_SEL_BRNO.HDNO = 'A1860') OR (GT_SEL_BRNO.HDNO = 'A1889') OR (GT_SEL_BRNO.HDNO = 'A1863')) then
                          ...
                          cxgWkCounsel.DataController.Values[iRow, iwkTitle[26]] := IfThen(ls_Rcrd[24] = 'm', '남자', '여자');
...
//												begin
                          ...
                          begin
                            cxgWkCounsel.DataController.Values[iRow, iwkTitle[63]] := '지정안됨';    //보험구분
													...
													else if ls_Rcrd[63] = 'b' then
													begin
														cxgWkCounsel.DataController.Values[iRow, iwkTitle[63]] := '개인보험';    //보험구분
													end;
...
//												cxgWkCounsel.DataController.Values[iRow, iwkTitle[71]] := -1;  //정렬사번
  ...
  {												sTmp := '';
                          ...
                          cxgWkCounsel.DataController.Values[iRow, iwkTitle[74]] := ls_Rcrd[75];//콜당 탁송보험 결과
													...
													begin
														cxgWkCounsel.DataController.Values[iRow, iwkTitle[80]] := ls_Rcrd[83];
													end;
													//ls_Rcrd[84] : 개인보험상태
													if ls_Rcrd.Count > 84 then
													begin
                            if ls_Rcrd[63] = 'b' then
                              cxgWkCounsel.DataController.Values[iRow, iwkTitle[81]] := ls_Rcrd[84]
                            else
                              cxgWkCounsel.DataController.Values[iRow, iwkTitle[81]] := ''; //개인보험일 경우에만 상태표기 20210318KHS 팀장님지시
													end;
													//ls_Rcrd[85] : 본사명  2022.02.24 추가
													if ls_Rcrd.Count > 85 then
													begin
                            cxgWkCounsel.DataController.Values[iRow, iwkTitle[82]] := ls_Rcrd[85];
                          end;
													//ls_Rcrd[86] : 면허종류
													if ls_Rcrd.Count > 86 then
													begin
                            cxgWkCounsel.DataController.Values[iRow, iwkTitle[83]] := ls_Rcrd[86];
                          end;
													//ls_Rcrd[87] : 수동가능(스틱여부)
													if ls_Rcrd.Count > 87 then
													begin
                            cxgWkCounsel.DataController.Values[iRow, iwkTitle[84]] := ls_Rcrd[87];
                          end;
												end;
                      finally
                        ls_Rcrd.Free;
...
begin
  ...
  begin
    meoWkList.Clear;
    meoWkList.Text := sTemp;
    lblState.Caption := '위의 기사님들을 [' + sTitle + '] 상태로 변경 하시겠습니까?';

    grpStateChange.Tag := TPopupMenu(Sender).Tag;
    grpStateChange.Left := (Width - grpStateChange.Width) div 2;
    grpStateChange.top := (Height - grpStateChange.Height) div 2;

    grpStateChange.BringToFront;
    grpStateChange.Visible := True;
  end;
end;
...
var
	sWk_sabun: string;
  iWkSabun, iCnt, i, iSendCnt, iLastCnt, iPass : Integer;
  slWkList : TStringList;
  iCheckCol: Integer;
begin
  iWkSabun := cxgWkCounsel.GetColumnByFieldName('기사사번').Index;
  iCnt := GetGridSelectedCount(cxgWkCounsel);
  if iCnt > 0 then
  begin
    iSendCnt := 500;   // 동시처리사원수를 입력 500. 동시에 500명씩 처리
    if iCnt Mod iSendCnt > 0 then iLastCnt := (iCnt Div iSendCnt) + 1
                             else iLastCnt := iCnt Div iSendCnt;

    if Length(sKey) = 1 then sKey := '0' + sKey;

    if iLastCnt > 1 then
    begin
   	grpBar.Caption := Format('작업 처리중 ( %d / %d )', [0, 0]);
  		PBar.Properties.Max := iLastCnt;
	  	PBar.Position := 0;
      grpBar.Left := (Self.Width - grpBar.Width) div 2;
      grpBar.Top  := (Self.Height - grpBar.Height) div 2;
      grpBar.BringToFront;
      grpBar.Visible := True;
    end;
    ...
    try
      ...
      begin
        ...
        begin
          ...
          slWkList.Add(sWk_sabun);
          lbWkSabun.Items.Add(sWk_sabun);
          if ( slWkList.Count = iSendCnt ) then
          begin
            Inc(iPass);
            if iLastCnt > 1 then
            begin
    		  		PBar.Position := iPass;
    			  	grpBar.Caption := Format('작업 처리중 ( %d / %d )', [iPass, iLastCnt]);
            end;
            proc_SelectSettlementExec(sKey, slWkList, iCnt = lbWkSabun.Items.Count, iLastCnt > 1);
            slWkList.Clear;
          end;
        end;
      end;

      if ( slWkList.Count > 0 ) then
      begin
        Inc(iPass);
        if iLastCnt > 1 then
        begin
...
procedure TFrm_WORA1.proc_SelectSettlementExec(sKey : string; AWkList : TStringList; bEnd, bDivision : Boolean);
var
	ls_TxLoad, sWk_sabun: string;
  rv_str, ls_MSG_Err, sMsg, sTmp : string;
  ls_rxxml: WideString;
...
begin
  ...
  try
    ...
    try
      ...
      try
        bFirst := False;
        for i := 0 to AWkList.Count - 1 do
        begin
          sWk_sabun := AWkList.Strings[i];
          if not bFirst then
          begin
            bFirst := True;
            // clone nodes ...
        ...
        begin
          ...
          begin
            ...
            begin
              if bDivision then
              begin
                sTmp := GetXmlErrorMsg(ls_rxxml); // 성공 : 3, 실패 : 0
                sTmp := Copy(sTmp, 1, Pos(',', sTmp)-1);
                sTmp := StringReplace(sTmp, '성공', '', [rfReplaceAll]);
                sTmp := StringReplace(sTmp, ':', '', [rfReplaceAll]);
                giSucc := giSucc + StrToIntDef(Trim(sTmp), 0);

                sTmp := GetXmlErrorMsg(ls_rxxml); // 성공 : 3, 실패 : 0
                sTmp := Copy(sTmp, Pos(',', sTmp)+1, Length(sTmp)-Pos(',', sTmp));
                sTmp := StringReplace(sTmp, '실패', '', [rfReplaceAll]);
                sTmp := StringReplace(sTmp, ':', '', [rfReplaceAll]);
                giFail := giFail + StrToIntDef(Trim(sTmp), 0);
              end;
              ...
              begin
                if bDivision then
                begin
                  GMessagebox('작업 완료' + CRLF + Format('성공 : %d, 실패 : %d', [giSucc, giFail]), CDMSE);
                end else
                begin
                  if iNodeNum > 0 then
                  begin
...
procedure TFrm_WORA1.cxButton9Click(Sender: TObject);
begin
  grpStateChange.Visible := False;
end;
	...
	try
		...
		begin
			...
			begin			         //[콜당보험2] - 0:가입요청, 1:통과, 2:거부, 3:심사중,  5:기타, 7:해지요청, 8:해지, D:재요청 [3차구분]
				if GS_PRJ_AREA = 'S' then
					Param := Param + '│' + sTmp + '│' + IntToStr(cbWKInsGubun.ItemIndex) //0:전체, 1:월정액, 2:콜당, 3:월당, 4 : 개인보험
				else
				begin
					//0:전체, 1:월정액, 2:콜당, 4:개인보험
					if cbWKInsGubun.ItemIndex = 3 then 
						Param := Param + '│' + sTmp + '│' + IntToStr(cbWKInsGubun.ItemIndex+1) 
					else
						Param := Param + '│' + sTmp + '│' + IntToStr(cbWKInsGubun.ItemIndex);
				...
				case cbWKInsGubun.ItemIndex of
				 0,1: Param := Param + '│' + ''                            + '│' + '';
					...
					3: //지방권은 개인보험이 cbWKInsGubun.ItemIndex 3 
					begin
						if GS_PRJ_AREA = 'S' then
							Param := Param + '│' + IntToStr(3)                   + '│' + sTmp1   //DB손보
						else 
							Param := Param + '│' + ''                            + '│' + ''//
					end;	
					4:  Param := Param + '│' + ''                            + '│' + ''//수도권은 개인보험이 cbWKInsGubun.ItemIndex 4
				end;
			end;
