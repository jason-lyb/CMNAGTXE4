...
//        sWhere := sWhere + Format(' AND IN_DATE || IN_TIME BETWEEN ''%s'' AND ''%s'' ', [sFromDate + LPAD(IntToStr(ASTime), '0', 2), sEndDate + LPAD(IntToStr(AETime), '0', 2)]);
//      end;
//    end else
    begin
      if cxDtStartA8.Enabled then
...
//        sEndDate  := FormatDateTime('yyyymmdd', cxDtEndA7.Date);
//        sWhere := sWhere + Format(' AND IN_DATE || IN_TIME BETWEEN ''%s09'' AND ''%s08'' ', [sFromDate, sEndDate])
//      end else
//      if Search_Gubun In [2, 3] then
//      begin
