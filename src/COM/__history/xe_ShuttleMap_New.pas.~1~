...
uses
  ...
  AdvObj, BaseGrid, AdvGrid, AdvCGrid, cxCheckBox, cxSplitter, cxCalendar,
  cxButtonEdit, cxShellComboBox, dxSkinOffice2010Silver, dxDateRanges, dxSkinSharp, dxSkinMetropolisDark, dxSkinOffice2007Silver;
...
type
  Tfrm_ShuttleMap_New = class(TForm)
    pnl_NaverMap: TPanel;
    pnlLargeMenu: TPanel;
    ...
    Shape4: TShape;
    cxLabel1: TcxLabel;
    edt_AddDownLine: TcxTextEdit;
    cxLabel3: TcxLabel;
    cxgDepotListColumn6: TcxGridBandedColumn;
		cxgDepotListColumn7: TcxGridBandedColumn;
    cb_Updown: TcxComboBox;
    Label6: TLabel;
    ...
    procedure cbLinePropertiesChange(Sender: TObject);
		procedure edt_AddUpLineKeyDown(Sender: TObject; var Key: Word;
      Shift: TShiftState);
    ...
    procedure edt_AddDownLineKeyDown(Sender: TObject; var Key: Word;
      Shift: TShiftState);
    procedure cb_UpdownPropertiesChange(Sender: TObject);
	private
		{ Private declarations }
		Naver_URL : string;

		gLineCode, gLineName, gLineUpName, gLineDownName : TStringList;
		slLon, slLat, slDepot, slDeCode, slDepMin, slDeUpDown : TStringList;
		...
		procedure proc_LineUpDownSearch(AGubun : integer);
		procedure setDepotPoiMark(aLatList, aLonList, aDepotList, aDepotCodeList, aDepMinList, aDeUpDownList : TStringList; aGubun : integer);
		procedure proc_MoveCenter(ALat, ALon, ALeavel : string);
		procedure proc_setFitBounds(aLatList, aLonList : TStringList);
		procedure CreateObj;
		procedure proc_LineSave(AGubun : integer); //0:save, 1:delete
...
var
	frm_ShuttleMap_New: Tfrm_ShuttleMap_New;

implementation

uses
	xe_gnl, xe_func, xe_msg, xe_WOR20, xe_Flash, xe_packet, xe_xml, xe_WOR21;

{$R *.dfm}

function Tfrm_ShuttleMap_New.Angle(Pt1, Pt2: TTmappos): Double;
var ang : Double;
 p1a, p2a : TPoint;
begin
...
function Tfrm_ShuttleMap_New.SK_GetAngleFromLine(pt1, pt2: TPoint): Double;
...
procedure Tfrm_ShuttleMap_New.trm_ESCTimer(Sender: TObject);
begin
	...
	cxgDepotListColumn6.Options.Editing := False;
	cxgDepotListColumn7.Options.Editing := False;
end;

procedure Tfrm_ShuttleMap_New.BtnCloseClick(Sender: TObject);
begin
{	if bEditing then
	begin
...
procedure Tfrm_ShuttleMap_New.btnLoadFromFileClick(Sender: TObject);
var
  I: Integer;
begin
	try
...
procedure Tfrm_ShuttleMap_New.btnShSchClick(Sender: TObject);
...
//response :  1.노선코드 2.노선명 
	...
	try
		...
		try
			...
			if not RequestBasePaging(GetSel06('GET_ROUTE_LIST', 'SHUTTLE_MNG.GET_ROUTE_LIST', '100', Param), slList, ErrCode, ErrMsg) then
			begin
						...
						begin
							...
							try
								...
								begin
									...
									gLineUpName.Add(ls_Rcrd[1]);
									gLineDownName.Add(ls_Rcrd[2]);
									gLineName.Add(ls_Rcrd[1] + '<->' + ls_Rcrd[2]);
								end;
...
procedure Tfrm_ShuttleMap_New.btn_AddClick(Sender: TObject);
var iRow : Integer;
	sCode : string;
begin
	...
	if cb_UpDown.ItemIndex <> 0 then
	begin
		GMessagebox('전체선택 후 추가하십시오', CDMSE);
		cb_UpDown.SetFocus;
		Exit;
	end;
...
//	cxgDepotList.DataController.FocusedRecordIndex := iRow;
	...
	cxgDepotList.DataController.Values[iRow, 5] := '1분';
//	if cb_Updown.itemIndex = 1 then sTmp := '상' else sTmp := '하';
	cxgDepotList.DataController.Values[iRow, 6] := '상';//sTmp;
	...
	cxgDepotListColumn6.Options.Editing := true;
	cxgDepotListColumn7.Options.Editing := true;
...
procedure Tfrm_ShuttleMap_New.btn_AddLineClick(Sender: TObject);
begin
	proc_AutoSearchStop; //자동갱신 초기화
	if bEditing then
	begin
...
procedure Tfrm_ShuttleMap_New.btn_AddLineCloseClick(Sender: TObject);
begin
	pnl_AddLine.Visible := false;
end;

procedure Tfrm_ShuttleMap_New.btn_AddLineSaveClick(Sender: TObject);
begin
	proc_LineSave(0);
end;

procedure Tfrm_ShuttleMap_New.btn_DeleteClick(Sender: TObject);
var iRow, i : Integer;
...
procedure Tfrm_ShuttleMap_New.btn_DelLineClick(Sender: TObject);
begin
	proc_AutoSearchStop; //자동갱신 초기화
	if bEditing then
	begin
...
procedure Tfrm_ShuttleMap_New.btn_DepotUpLoadClick(Sender: TObject);
begin
	proc_AutoSearchStop; //자동갱신 초기화
...
procedure Tfrm_ShuttleMap_New.btn_ExcelCloseClick(Sender: TObject);
begin
	pnl_AddDepot.Visible := false;
	proc_BtnEnabled(True, 'Depot');
end;

procedure Tfrm_ShuttleMap_New.btn_ExcelDepotSaveClick(Sender: TObject);
var sMsg : string;
begin
	Try
...
procedure Tfrm_ShuttleMap_New.btn_LineSearchClick(Sender: TObject);
begin
	try
		proc_AutoSearchStop; //자동갱신 초기화
...
procedure Tfrm_ShuttleMap_New.btn_SaveClick(Sender: TObject);
var sMsg : string;
begin
	Try
...
procedure Tfrm_ShuttleMap_New.btn_ShuttleClick(Sender: TObject);
begin
{	if bEditing then
	begin
		cxgDepotListColumn2.Options.Editing := False;	
...
//		//셀선택 처리
 ...
 //		Exit;    
	...
	if Not Assigned(Frm_WOR21) Or (Frm_WOR21 = Nil) then Frm_WOR21 := TFrm_WOR21.Create(Nil);
	Frm_WOR21.proc_init;
	Frm_WOR21.gLineCode.Assign(gLineCode);
	Frm_WOR21.gLineName.Assign(gLineName);
	Frm_WOR21.gLineUpName.Assign(gLineUpName);
	Frm_WOR21.gLineDownName.Assign(gLineDownName);
	Frm_WOR21.cbLine.Properties.items.Assign(cbLine.Properties.items);
	Frm_WOR21.cbLine.ItemIndex := cbLine.ItemIndex;

	if gShuttleBrNo = '' then
	begin
		Frm_WOR21.btnLineNameUpdate.enabled := False;
	end else
	begin
		Frm_WOR21.btnLineNameUpdate.enabled := True;
	end;
	Frm_WOR21.gShuttleBrNo := gShuttleBrNo;    
	Frm_WOR21.Top := self.Top;// + pnlTitle.Height;
	Frm_WOR21.Show;
end;

procedure Tfrm_ShuttleMap_New.btn_UpClick(Sender: TObject);
var iSelfRow, iSomeoneRow, iTag : integer;
	sSelf_1, sSelf_2, sSelf_3, sSelf_4, sSelf_5, sSelf_6, sSelf_7 : string; 
	sSomeone_1, sSomeone_2, sSomeone_3, sSomeone_4, sSomeone_5, sSomeone_6, sSomeone_7 : string; 
begin
	proc_AutoSearchStop; //자동갱신 초기화
	...
	sSelf_6 :=  cxgDepotList.DataController.Values[iSelfRow, 5];        //경과시간
	sSelf_7 :=  cxgDepotList.DataController.Values[iSelfRow, 6];        //상하행
	...
	sSomeone_6 :=  cxgDepotList.DataController.Values[iSomeoneRow, 5]; //경과시간
	sSomeone_7 :=  cxgDepotList.DataController.Values[iSomeoneRow, 6]; //상하행
	...
	cxgDepotList.DataController.Values[iSomeoneRow, 5] := sSelf_6;
	cxgDepotList.DataController.Values[iSomeoneRow, 6] := sSelf_7;
	...
	cxgDepotList.DataController.Values[iSelfRow, 5] := sSomeone_6;
	cxgDepotList.DataController.Values[iSelfRow, 6] := sSomeone_7;
...
procedure Tfrm_ShuttleMap_New.btn_UpdateClick(Sender: TObject);
var iRow : Integer;
	sCode : string;
begin
	proc_AutoSearchStop; //자동갱신 초기화
...
procedure Tfrm_ShuttleMap_New.cbLinePropertiesChange(Sender: TObject);
begin
	if bEditing then
	begin
		GMessagebox('정류장 추가/수정 중입니다. 정류장 추가/수정을 완료 하십시오(저장-Enter, 취소-ESC)', CDMSE);
...
procedure Tfrm_ShuttleMap_New.cb_SUpDateClick(Sender: TObject);
var
  iCnt: integer;
begin
	if bEditing then
...
procedure Tfrm_ShuttleMap_New.cb_UpdownPropertiesChange(Sender: TObject);
begin
	if (not bEditing) and (cb_UpDown.Tag = 0) then
		proc_LineUpDownSearch(cb_Updown.ItemIndex);
end;

procedure Tfrm_ShuttleMap_New.CreateObj;
var i : integer;
...
procedure Tfrm_ShuttleMap_New.cxgDepotListCellClick(Sender: TcxCustomGridTableView;
	ACellViewInfo: TcxGridTableDataCellViewInfo; AButton: TMouseButton;
  AShift: TShiftState; var AHandled: Boolean);
begin
//	cxGridDepot.SetFocus;
...
procedure Tfrm_ShuttleMap_New.cxgDepotListCellDblClick(
...
//	proc_AutoSearchStop; //자동갱신 초기화
	iRow := cxgDepotList.DataController.FocusedRecordIndex;
	if iRow < 0 then exit;
	sLat := cxgDepotList.DataController.Values[iRow, 2];
	sLon := cxgDepotList.DataController.Values[iRow, 3];

	proc_MoveCenter(sLat, sLon, '13');
//	proc_MoveCenter(slLat[iRow], slLon[iRow], '13');
{	Try
...
procedure Tfrm_ShuttleMap_New.cxgDepotListEditKeyDown(
	Sender: TcxCustomGridTableView; AItem: TcxCustomGridTableItem;
	AEdit: TcxCustomEdit; var Key: Word; Shift: TShiftState);
var sMsg, sDepot, sMin : string;
	slatX, slonY : String;
begin
	try
	...
	//			sDepot := cxgDepotList.DataController.Values[gPopRow, 1];
				...
				begin
					...
					if cxgDepotList.ViewData.Records[gPopRow].Values[5] = null then sMin := '' else
					sMin := cxgDepotList.ViewData.Records[gPopRow].Values[5];

					if Trim(sMin) = '' then
					begin
						GMessagebox('소요시간을 선택하세요', CDMSE);
						cxgDepotListColumn2.Options.Editing := True;	
						cxgDepotListColumn6.Options.Editing := True;	
						cxgDepotListColumn7.Options.Editing := True;	
						bEditing := cxgDepotListColumn2.Options.Editing;
...
procedure Tfrm_ShuttleMap_New.cxgDepotListFocusedRecordChanged(
...
procedure Tfrm_ShuttleMap_New.cxgDepotListKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
	Try
...
procedure Tfrm_ShuttleMap_New.edt_AddDownLineKeyDown(Sender: TObject;
  var Key: Word; Shift: TShiftState);
begin
	if key = vk_Return then btn_AddLineSave.click;
end;

procedure Tfrm_ShuttleMap_New.edt_AddUpLineKeyDown(Sender: TObject; var Key: Word;
	Shift: TShiftState);
begin
	if key = vk_Return then edt_AddDownLine.SetFocus;
	
end;


function Tfrm_ShuttleMap_New.ExcelToGridEx(AFilename: string;
...
procedure Tfrm_ShuttleMap_New.FormActivate(Sender: TObject);
begin
  cxLblActive.Color := GS_ActiveColor;
  cxLblActive.Visible := True;
end;

procedure Tfrm_ShuttleMap_New.FormClose(Sender: TObject; var Action: TCloseAction);
begin
	...
	if Assigned(slDeUpDown) then slDeUpDown.Free;
	if Assigned(slDepMin) then slDeUpDown.Free;
...
procedure Tfrm_ShuttleMap_New.FormCreate(Sender: TObject);
begin
	try
		CreateObj;
		AutoFlag := False;
    ...
    begin
      Self.Caption := '  셔틀버스기사등록';
      pnlTitle.caption := Self.Caption;
    end;
		...
		cxgDepotList.Columns[5].DataBinding.ValueType := 'String';
		cxgDepotList.Columns[6].DataBinding.ValueType := 'String';
		...
		slDeUpDown := TStringList.Create;
		slDepMin := TStringList.Create;
...
procedure Tfrm_ShuttleMap_New.FormDeactivate(Sender: TObject);
begin
  cxLblActive.Visible := False;
end;

procedure Tfrm_ShuttleMap_New.FormDestroy(Sender: TObject);
begin
	frm_ShuttleMap_New := Nil;
end;

procedure Tfrm_ShuttleMap_New.FormResize(Sender: TObject);
begin
	SetDebugeWrite('frm_ShuttleMap_New.FormResize');
	try
		if not FResize then
		begin
			if not Assigned(frm_ShuttleMap_New) then
			begin
...
procedure Tfrm_ShuttleMap_New.FormShow(Sender: TObject);
begin
  fSetFont(frm_ShuttleMap_New, GS_FONTNAME);
	...
	if GT_SEL_BRNO.BRNO = 'S316' then	SHUTTLE_BrNo := 'S316';
...
procedure Tfrm_ShuttleMap_New.mn_AddClick(Sender: TObject);
begin
	btn_AddClick(btn_Add);	
end;

procedure Tfrm_ShuttleMap_New.mn_DeleteClick(Sender: TObject);
begin
	btn_DeleteClick(btn_Delete);
end;

procedure Tfrm_ShuttleMap_New.mn_UpdateClick(Sender: TObject);
begin
	btn_UpdateClick(btn_Update);

end;

procedure Tfrm_ShuttleMap_New.Panel1MouseDown(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
	ReleaseCapture;
...
procedure Tfrm_ShuttleMap_New.pLoadNaverMap;
Var iRandom : integer;
		sClient: string;
begin
	Try
...
//		Naver_URL := 'http://203.251.202.13:88/map/makeNaverMap_Shuttle_New.jsp?clientId=' + sClient;
		Naver_URL := 'http://203.251.202.13:88/map/makeNaverMap_Shuttle_New_cloud.jsp?ncpClientId=' + sClient;
		bWebLoading := False;
		try
...
procedure Tfrm_ShuttleMap_New.pmEditPopup(Sender: TObject);
begin
...
procedure Tfrm_ShuttleMap_New.pnlTitleMouseDown(Sender: TObject;
...
procedure Tfrm_ShuttleMap_New.pnl_AddLineMouseDown(Sender: TObject;
	Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
begin
	ReleaseCapture;
...
procedure Tfrm_ShuttleMap_New.proc_setFitBounds(aLatList, aLonList : TStringList);
...
procedure Tfrm_ShuttleMap_New.setDepotPoiMark(aLatList, aLonList,
	aDepotList, aDepotCodeList, aDepMinList, aDeUpDownList: TStringList; aGubun : integer);
var i : integer;
	sTmp, sGubun : string;
	sl_Number, slLatList, slLonList,
	slDepotList, slDepotCodeList, slDepMinList, slDeUpDownList : TStringList;
begin
	Try
		sl_Number := TStringList.Create; slLatList := TStringList.Create;	slLonList := TStringList.Create;
		slDepotList := TStringList.Create; slDepotCodeList := TStringList.Create;
		slDepMinList := TStringList.Create; slDeUpDownList := TStringList.Create;

		if aGubun = 1 then
		begin
			for i := 0 to aLatList.Count - 1 do
			begin
				if aDeUpDownList[i] <> 'U' then Continue;
				sl_Number      .add(intToStr(i+2));
				slLatList			 .add(aLatList[i]);
				slLonList			 .add(aLonList[i]);
				slDepotList		 .add(aDepotList[i]);
				slDepotCodeList.add(aDepotCodeList[i]);
				slDepMinList   .add(aDepMinList[i]);
				slDeUpDownList .add(aDeUpDownList[i]);
			end;
		End else if aGubun = 2 then
		begin
			for i := 0 to aLatList.Count - 1 do
			begin
				if aDeUpDownList[i] <> 'D' then Continue;
				sl_Number      .add(intToStr(i+2));
				slLatList			 .add(aLatList[i]);
				slLonList			 .add(aLonList[i]);
				slDepotList		 .add(aDepotList[i]);
		...
		Try
			for i := 0 to slLatList.Count - 1 do
			begin
				if slDeUpDownList[i] = 'U' then sGubun := '상' else sGubun := '하';
				sTmp := slDepotList[i] + '/' + slDepMinList[i] + '분/' + sGubun;
				ExecuteJavaScript(webNaverMap,'setDepotPoiMark('+sl_Number[i]+', '+slLatList[i]+','+ slLonList[i] +','''+ sTmp +''','''+ slDepotCodeList[i] +''');');
			end;
    Finally
			sl_Number.Free; slLatList.Free;	slLonList.Free; slDepotList.Free; slDepotCodeList.Free;	slDepMinList.Free; slDeUpDownList.Free;
		End;
{		for i := 0 to aLatList.Count - 1 do
		begin
			if aDeUpDownList[i] = 'U' then sGubun := '상' else sGubun := '하';
			sTmp := aDepotList[i] + '/' + aDepMinList[i] + '분/' + sGubun;
			ExecuteJavaScript(webNaverMap,'setDepotPoiMark('+InttoStr(i+2)+', '+aLatList[i]+','+ aLonList[i] +','''+ sTmp +''','''+ aDepotCodeList[i] +''');');
		end;  }
...
procedure Tfrm_ShuttleMap_New.tmr_SdisTimer(Sender: TObject);
begin
	try
		if (cb_SUpDate.ItemIndex = 0) then
		begin
...
procedure Tfrm_ShuttleMap_New.proc_AutoSearchStop;
begin
	cb_SUpDate.ItemIndex := 0;
	tmr_Sdis.Enabled := False;
	cxCESdt.Value := 0; // 갱신시간 카운트.
...
procedure Tfrm_ShuttleMap_New.proc_BtnEnabled(AEnable: Boolean; AType : string);
begin
...
procedure Tfrm_ShuttleMap_New.proc_BusSearch;
var XmlData, Param, ErrMsg, ls_MSG_Err : string;
	ErrCode, i : Integer;
	xdom: msDomDocument;
	lst_Result : IXMLDomNodeList;
...
// response :  1.기사사번 2.기사명 3.단말기번호 4.이전좌표 X 5.이전좌표 Y 6.좌표 X 7.좌표 Y
	...
	try
		...
		try
			...
			if not RequestBasePaging(GetSel06('GET_DRIVE_SHUTTLE_WK_LIST', 'SHUTTLE_MNG.GET_DRIVE_SHUTTLE_WK_LIST', '100', Param), slList, ErrCode, ErrMsg) then
			begin
...
//								Log('관리이전좌표 : ' + slTmp4[i] + '|' +  slTmp5[i], LOGDATAPATHFILE);
								...
								sTmp := slTmp1[i] + '[' + slTmp2[i] + '-' + slTmp3[i] + ']';
...
procedure Tfrm_ShuttleMap_New.proc_DepotSave(AGubun : string);
var sParam, sDepCode, sDepName, sLon, sLat, sMin, sUpDown, sTmp : string;
	i : integer;
	...
	try
		...
		begin
			...
			begin
				...
				begin
					...
					sMin := StringReplace(cxgDepotList.DataController.Values[i, 5],'분', '', [rfReplaceAll]);
					sTmp := cxgDepotList.DataController.Values[i, 6];
					if sTmp = '상' then sTmp := 'U' else sTmp := 'D';
					sUpDown := sTmp;
				end
				else
				begin
					...
					sMin := sMin + ',' + StringReplace(cxgDepotList.DataController.Values[i, 5],'분', '', [rfReplaceAll]);
					sTmp := cxgDepotList.DataController.Values[i, 6];
					if sTmp = '상' then sTmp := 'U' else sTmp := 'D';
					sUpDown := sUpDown + ',' + sTmp;
				end;
			end;
		end else
        ...
        if i = 100 then break;
				...
				begin
					...
					sMin := StringReplace(cxViewFileExcelList.DataController.Values[i, 3],'분', '', [rfReplaceAll]);
					sTmp := cxViewFileExcelList.DataController.Values[i, 4];
					if sTmp = '상' then sTmp := 'U' else sTmp := 'D';
					sUpDown := sTmp;
				end
				else
				begin
					...
					if pos('.', sTmp) < 1 then sTmp := SetWGS84(sTmp);
					sLon := sLon + ',' + sTmp;
					sMin := sMin + ',' + StringReplace(cxViewFileExcelList.DataController.Values[i, 3],'분', '', [rfReplaceAll]);
					sTmp := cxViewFileExcelList.DataController.Values[i, 4];
					if sTmp = '상' then sTmp := 'U' else sTmp := 'D';
					sUpDown := sUpDown + ',' + sTmp;
				end;
		...
		sParam := sParam + '│' + sDepCode + '│' + sDepName + '│' + sLat + '│' + sLon + '│' + sMin + '│' + sUpDown;		

		
		if not RequestBase(GetCallable06('SET_STATION', 'SHUTTLE_MNG.SET_STATION', sParam), XmlData, ErrCode, ErrMsg) then
		begin
...
procedure Tfrm_ShuttleMap_New.proc_Init(AType: Integer);
begin
	gPopRow := -1;
	gClickRow := -1;
	...
	cb_UpDown.Tag := 99;
	cb_UpDown.ItemIndex := 0;
	cb_UpDown.Tag := 0;
end;

procedure Tfrm_ShuttleMap_New.proc_LineSave(AGubun : integer); //0:save, 1:delete;
var sHdNo, sBrNo, sParam, sMsg : string;
	XmlData, ErrMsg : string;
	ErrCode, iDx : integer;
begin
...
//response : 성공,실패 (기존그대로)
	try
		...
		if AGubun = 0 then
		begin
			if Trim(edt_AddUpLine.Text) = '' then
			begin
				GMessagebox('상행노선명을 입력하세요.', CDMSE);
				edt_AddUpLine.SetFocus;
				exit;
			end;
			if Trim(edt_AddDownLine.Text) = '' then
			begin
				GMessagebox('하행노선명을 입력하세요.', CDMSE);
				edt_AddDownLine.SetFocus;
				exit;
			end;
			...
			sParam := '';
			sParam := IntToStr(AGubun) + '│' + sHdNo + '│' + sBrNo + '│' + GT_USERIF.ShareNo + '│' 
							+ '' + '│' + Trim(edt_AddUpLine.Text) + '│' + Trim(edt_AddDownLine.Text);

			if not RequestBase(GetCallable06('SET_ROUTE_SAVE', 'SHUTTLE_MNG.SET_ROUTE_SAVE', sParam), XmlData, ErrCode, ErrMsg) then
			begin
			...
			iDx := cbLine.Properties.Items.IndexOf(edt_AddUpLine.text + '<->' + edt_AddDownLine.Text);
		...
		begin
			...
			sParam := '';
			sParam := IntToStr(AGubun) + '│' + '' + '│' + '' + '│' + '' + '│' 
							+ gLineCode[cbLine.ItemIndex] + '│' + '' + '│' + '';

			sMsg := '[' + cbLine.Text + '] 노선을 삭제하시겠습니까?';
			if Application.MessageBox(PChar(sMsg), '[노선삭제]', MB_YESNO +	MB_ICONQUESTION + MB_DEFBUTTON1) = IDYES then
			begin
				if not RequestBase(GetCallable06('SET_ROUTE_DELETE', 'SHUTTLE_MNG.SET_ROUTE_SAVE', sParam), XmlData, ErrCode, ErrMsg) then
				begin
...
procedure Tfrm_ShuttleMap_New.proc_LineSearch(ASraechType : string);
var XmlData, Param, ErrMsg, ls_MSG_Err : string;
		...
		try
						...
						begin
							...
							try
								...
								begin
									...
									slLat   .Add(ls_Rcrd[2]);
									slLon   .Add(ls_Rcrd[3]);
									slDepMin.Add(ls_Rcrd[4]);
									slDeUpDown.Add(ls_Rcrd[5]);
								end;
							finally
								ls_Rcrd.Free;
							end;
			...
			Try
				...
				begin
					...
					setDepotPoiMark(slLat, slLon, slDepot, slDeCode, slDepMin, slDeUpDown, -1);
					...
					begin
						...
						cxgDepotList.DataController.Values[iRow, 5] := slDepMin[i] + '분';
						if slDeUpDown[i] = 'U' then
							cxgDepotList.DataController.Values[iRow, 6] := '상'
						else
							cxgDepotList.DataController.Values[iRow, 6] := '하';
					end;
					cxgDepotList.EndUpdate;	
				end
				else 
...
procedure Tfrm_ShuttleMap_New.proc_LineUpDownSearch(AGubun: integer);
var
	iRow, i : Integer;
	slList : TStringList;
begin
...
// response :  1.정류장순번 2.정류장명 3.X 4.Y 정5.류장이동소요시간 분단위 6.상하행구분 (상U , 하 D) 
	...
	try
		...
		begin
			...
			setDepotPoiMark(slLat, slLon, slDepot, slDeCode, slDepMin, slDeUpDown, AGubun);
			...
			begin
				if AGubun = 1 then 
				begin
					if slDeUpDown[i] <> 'U' then Continue;
				end else
				if AGubun = 2 then 
				begin
					if slDeUpDown[i] <> 'D' then Continue;
				end;
				
				iRow := cxgDepotList.DataController.AppendRecord;
				cxgDepotList.DataController.Values[iRow, 0] := I+1;
				cxgDepotList.DataController.Values[iRow, 1] := slDepot[i];
				cxgDepotList.DataController.Values[iRow, 2] := slLat[i];
				cxgDepotList.DataController.Values[iRow, 3] := slLon[i];
				cxgDepotList.DataController.Values[iRow, 4] := slDeCode[i];
...
procedure Tfrm_ShuttleMap_New.proc_MoveCenter(ALat, ALon, ALeavel: string);
begin
	Try
...
procedure Tfrm_ShuttleMap_New.webNaverMapDocumentComplete(ASender: TObject;
  const pDisp: IDispatch; const URL: OleVariant);
begin
...
procedure Tfrm_ShuttleMap_New.webNaverMapNavigateComplete2(ASender: TObject;
	const pDisp: IDispatch; const URL: OleVariant);
begin
