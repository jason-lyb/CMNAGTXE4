...
function FastPos(const aSourceString, aFindString : AnsiString; const aSourceLen, aFindLen, StartPos : Integer) : Integer;
...
function FastReplace(const aSourceString : AnsiString; const aFindString, aReplaceString : AnsiString; CaseSensitive : Boolean = False) : AnsiString;
...
begin
  ...
  Result := Integer(BMPos(PAnsiChar(aSourceString) + (StartPos - 1), PAnsiChar(aFindString),aSourceLen - (StartPos-1), aFindLen, JumpTable));
  if Result > 0 then
