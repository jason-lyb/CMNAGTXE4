...
Type
  ...
  TSubClass_Proc = function(lng_hWnd: HWND; uMsg: Integer; var bGlue:Boolean;
  ...
  TMagnetic = class
   ...
   private
    ...
    procedure pvMoveRect(Handle: HWND; var rcWnd: TRECT; bGlue : Boolean);
    procedure pvCheckGlueing(bGlue: Boolean);
...
begin
   ...
   begin
      MagneticWnd.zSubclass_Proc(lng_hWnd, uMsg,
                                 Msg.wParam, Msg.lParam, Msg.Result, bHandled, bGlue);
      result := true;
   end else
      result := false;
end;
...
begin
    ...
    Case uMsg of
        ...
        WM_MOVING:
          begin
            CopyMemory(@rcWnd, pointer(lParam), LB_RECT);
            pvMoveRect(lng_hWnd, rcWnd, bGlue);
            CopyMemory(pointer(lParam), @rcWnd, LB_RECT);
...
procedure TMagnetic.pvMoveRect(Handle: HWND; var rcWnd: TRECT; bGlue : Boolean);
var
  lc1:   integer;
  lc2:   integer;
  lWId:  integer;
...
begin
  ...
  begin
    ...
    with m_uWndInfo[lc1] do
      ...
      if (hWndParent = Handle) and (bGlue) then
      begin
        // Move 'child' window
        ...
        // 서로 SnapWidth이상 떨어져 있으면 자석효과 해제
        if (m_rcWnd[lc1].Top < m_rcWnd[lWId].Bottom + SnapWidth) and (m_rcWnd[lc1].Bottom > m_rcWnd[lWId].Top - SnapWidth) then
        begin
          case Abs(m_rcWnd[lc1].Left - m_rcWnd[lWId].Left) < SnapWidth of
            True:
              iDiff := m_rcWnd[lWId].Left - m_rcWnd[lc1].Left;
          end;
          case Abs(m_rcWnd[lc1].Left - m_rcWnd[lWId].Right) < SnapWidth of
            True:
              iDiff := m_rcWnd[lWId].Right - m_rcWnd[lc1].Left;
          end;
          case Abs(m_rcWnd[lc1].Right - m_rcWnd[lWId].Left) < SnapWidth of
            True:
              iDiff := m_rcWnd[lWId].Left - m_rcWnd[lc1].Right;
          end;
          case Abs(m_rcWnd[lc1].Right - m_rcWnd[lWId].Right) < SnapWidth of
            True:
              iDiff := m_rcWnd[lWId].Right - m_rcWnd[lc1].Right;
          end;
        end;

        // Y magnetism
        if (m_rcWnd[lc1].Left < m_rcWnd[lWId].Right + SnapWidth) and (m_rcWnd[lc1].Right > m_rcWnd[lWId].Left - SnapWidth) then
        begin
          case Abs(m_rcWnd[lc1].Top - m_rcWnd[lWId].Top) < SnapWidth of
            True:
              iDiff := m_rcWnd[lWId].Top - m_rcWnd[lc1].Top;
          end;
          case Abs(m_rcWnd[lc1].Top - m_rcWnd[lWId].Bottom) < SnapWidth of
            True:
              iDiff := m_rcWnd[lWId].Bottom - m_rcWnd[lc1].Top;
          end;
          case Abs(m_rcWnd[lc1].Bottom - m_rcWnd[lWId].Top) < SnapWidth of
            True:
              iDiff := m_rcWnd[lWId].Top - m_rcWnd[lc1].Bottom;
          end;
          case Abs(m_rcWnd[lc1].Bottom - m_rcWnd[lWId].Bottom) < SnapWidth of
            True:
              iDiff := m_rcWnd[lWId].Bottom - m_rcWnd[lc1].Bottom;
          end;
        end;

        if iDiff < SnapWidth then bCheck := True;
...
procedure TMagnetic.pvCheckGlueing(bGlue: Boolean);
var
  lcMain: integer;
  lc1:    integer;
  lc2:    integer;
