...
const
    ...
    , (FieldName: '경유지';	        TagName: '')
		...
		, (FieldName: '지원금';         TagName: 'SupportCharge')
	);
...
type
  TFrm_JON012 = class(TForm)
    ...
    cxGVeOrderListColumn44: TcxGridDBColumn;
    cxGVeOrderDestListColumn7: TcxGridDBColumn;procedure btnCloseClick(Sender: TObject);
    ...
    try
      ...
      try
        ...
        begin
          ...
          if sTmp <> '' then
          begin
            iRow := cxGVeOrderList.GetColumnByFieldName(sTmp).Index;
            if iRow = -1 then iRow := i;
          end
          else
            iRow := i;
          cxGVeOrderList.Columns[iRow].Index := i;
          cxGVeOrderList.Columns[iRow].DataBinding.ValueType := IfThen(sTmp = 'No', 'Integer', 'String');
...
//									if iTmp > 0 then Continue;
									case J of
									 ...
									 49: //지원금
											begin
                        sTmp := lst_Result.item[iCnt - 1].attributes.getNamedItem('SupportCharge').Text;
												DataValue := FormatFloat('#,##0', StrToFloatDef(sTmp, 0));
											end;
										6:
											begin
