  ...
  begin
    ...
    begin
      ...
      if getNamedItem('br_biz_name').text = 'VAL_br_biz_name' then
        ...
        edt_biz_Name.Text := getNamedItem('br_biz_name').text;
      lb_BaseSNm.Hint := edt_biz_Name.Text;
      ...
      edt_Emp_no.Text := getNamedItem('br_emp_no').text;
...
begin
  ...
  // 지사 사용안함일경우 필수 항목 체크 안함 2022.01.20. 명재대리 요청
  if rbBaseBrUseN.Checked then
  begin
    Result := True;
    Exit;
  end;
...
//	end;
  ...
  begin
    ...
    if Not chk_BizNumUse.Checked then
...
begin
  try
    ...
    ls_TxLoad := StringReplace(ls_TxLoad, 'VAL_head_name', En_Coding(AData.head_name), [rfReplaceAll]);
    ...
    ls_TxLoad := StringReplace(ls_TxLoad, 'VAL_name', En_Coding(AData.name), [rfReplaceAll]);
    ...
    ls_TxLoad := StringReplace(ls_TxLoad, 'VAL_address2', AData.address2, [rfReplaceAll]);
		...
		ls_TxLoad := StringReplace(ls_TxLoad, 'VAL_kidi_biz_name', En_Coding(AData.BrName), [rfReplaceAll]);
		ls_TxLoad := StringReplace(ls_TxLoad, 'VAL_kidi_biz_num', AData.BrSnum, [rfReplaceAll]);
		ls_TxLoad := StringReplace(ls_TxLoad, 'VAL_br_biz_name', En_Coding(AData.biz_Name), [rfReplaceAll]);
...
begin
  ...
  begin
    ...
    edt_biz_Name.Enabled := False;
    edt_biz_Name.Clear;
  end else
  begin
    edtBaseSsn.Enabled := True;
    edtBaseSsn.text := lb_BaseSsn.Hint;
    edt_biz_Name.Enabled := True;
    edt_biz_Name.text := lb_BaseSNm.Hint;
  end;
end;
